...
# db_ra <- db2 %>% 
#   group_by(country) %>% 
#   mutate(ra = rollapply(new_c_pcp, 14, mean, align = 'center', fill = NA)) %>% 
#   ungroup()
#   
# db_ra %>%
...
db2 <- db1 %>% 
  group_by(country) %>%
  mutate(new_c_pcp_sm = rollapply(new_c_pcp, 14, mean, align = 'center', fill = NA),
...
db_t2 <- db_t %>% 
  ...
  select(country, date, positive_rate, new_tests, new_cases, new_tests_smoothed, total_tests, new_cases_smoothed) %>% 
  filter(country %in% ctrs,
         date >= "2020-03-01") %>% 
  mutate(date = ymd(date),
         # pos = as.numeric(positive_rate),
         pos = as.numeric(new_cases_smoothed) / as.numeric(new_tests_smoothed),
...
db_t3 <- db_t2 %>% 
  group_by(country) %>%
  mutate(pos_sm = rollapply(pos, 14, mean, align = 'center', fill = NA)) %>%
  ungroup()


db_t2 %>%
  mutate(country = factor(country, levels = ctrs)) %>% 
  ggplot(aes(date, pos, col = country))+
  geom_line(size = .5, alpha = .9) +
