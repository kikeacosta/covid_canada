...
db_decomp2 %>% 
  ...
  labs(title = paste0("Decomposition of CFR change between April 15 and July 16"),
       x = "Province",
       y = "CFR difference")+
  theme_bw()+
  coord_flip()+
...
ggsave("Figures/cfr_time_diff1.png", width = 5, height = 2)
...
for(c in c("Alberta", "BC", "Montreal", "Ontario", "Quebec")){
  ...
  db_decomp <- apply_kitagawa(db_d2, db_d3) %>% 
    mutate(Region = c,
           Date1 = d2,
           Date2 = d3) %>% 
    select(Region, Date1, Date2, everything()) %>% 
...
db_decomp3 %>% 
  ...
  scale_color_manual(values = c("#43a2ca", "#e34a33"), labels = c("Age structure", "Fatality"))+
  scale_fill_manual(values = c("#43a2ca", "#e34a33"), labels = c("Age structure", "Fatality"))+
  labs(title = paste0("Decomposition of CFR change between July 16 and October 15"),
       x = "Province",
       y = "CFR difference")+
  theme_bw()+
  coord_flip()+
...
ggsave("Figures/cfr_time_diff2.png", width = 5, height = 2)
